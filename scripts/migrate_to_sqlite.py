...
Usage:
    ...
    python scripts/migrate_to_sqlite.py --db-url "postgresql://user:pass@host:port/dbname"
"""
import os
import sys
import sqlite3
...
SQLITE_DB_PATH = os.path.join(REPO_ROOT, "quora_answers.db")
...
def main():
    ...
    if not database_url:
        ...
        print("  2. Pass via command line: python scripts/migrate_to_sqlite.py --db-url 'postgresql://...'")
        print("\nExample for Supabase:")
        print(
            "  python scripts/migrate_to_sqlite.py --db-url 'postgresql://postgres.xxx:PASSWORD@aws-1-ap-southeast-1.pooler.supabase.com:6543/postgres'"
        )
        return
