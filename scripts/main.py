...
def main():
    ...
    setup_db = os.path.join(REPO_ROOT, "scripts", "setup_database.py")
    run_scraper = os.path.join(REPO_ROOT, "scripts", "run_scraper.py")
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                subprocess.run([sys.executable, setup_db], check=False)
                print()
                ...
                if confirm.lower() == "y":
                    subprocess.run([sys.executable, run_scraper, "--mode", "collect"], check=False)
                print()
                ...
                confirm = input("Continue? (y/N): ")
                if confirm.lower() == "y":
                    subprocess.run([sys.executable, run_scraper, "--mode", "process"], check=False)
                print()
...
try:
    ...
    if incomplete_count > 0:
        ...
        print('python scripts/run_scraper.py --mode process')
